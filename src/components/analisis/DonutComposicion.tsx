...
import { COLORES, PALETA_DONUT } from './colores'
  ...
  return (
    <div className="max-w-72 rounded-lg border border-ciruela-700 bg-ciruela-950 px-3 py-2 text-xs shadow-xl">
      <p className="font-semibold text-white">
        {porcion.nombre}: <span className="font-mono">{moneda(porcion.valor)}</span>
      </p>
      {porcion.topCuentas.length > 0 && (
        <>
          <p className="mt-1.5 text-ciruela-400">Principales cuentas:</p>
          <ul className="mt-0.5 space-y-0.5">
            {porcion.topCuentas.map((c) => (
              <li key={c.nombre} className="text-ciruela-200">
                {c.nombre}: <span className="font-mono">{monedaCompacta(c.valor)}</span>
              </li>
            ))}
          </ul>
        </>
...
export default function DonutComposicion({ titulo, porciones }: DonutComposicionProps) {
  ...
  return (
    <div className="rounded-2xl border border-ciruela-800 bg-ciruela-900/60 p-4">
      <h2 className="mb-1 text-sm font-semibold text-white">{titulo}</h2>
      {conDatos.length === 0 ? (
        <p className="py-16 text-center text-xs text-ciruela-400">Sin datos para el mes seleccionado.</p>
      ) : (
        <>
          <p className="mb-2 text-xs text-ciruela-400">
            Total: <span className="font-mono text-ciruela-200">{moneda(total)}</span>
          </p>
