...
export default function Externos() {
  ...
  // Nombre a mostrar del vínculo de natillera (badge), por id.
  ...
  return (
    <div>
      ...
      <div className="mt-1 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-brand-900">{t.externos.titulo}</h1>
      ...
      {cargando && <p className="mt-6 text-sm text-tinta-suave">{t.externos.cargando}</p>}
      ...
      {!cargando && !externosQuery.error && (
        <>
          ...
          {lista.length > 0 && (
            <p className="mt-3 text-xs text-tinta-suave">{t.externos.notaPie}</p>
          )}
        </>
      )}
